...
function getRequireMock(originalFile, optionalMockSpecs, optionalCache) {
	...
	function interceptRequire(lib, optional__filename, optional__dirname) {
		...
		if (!cache[filePath]) {
			...
			var dirname =  optional__dirname ?  JSON.stringify(optional__dirname) : JSON.stringify(path.dirname(filePath));
			var tmpVar = "requireMock_tmp_" + Date.now() + "_" + (Math.random().toString().replace(".", ""));
			var code = "global." + tmpVar + " = (function (){" +
				"var __filename = " + filename + ";" +
				"var __dirname = " + dirname + ";" +
				"var require = __requireMock_originalRequire(" + JSON.stringify(__filename) + ")(" + JSON.stringify(filePath) + ", global.__mockSpecs, global.__requireMock_cache);" +
